...
public class AppController {
	...
	@ResponseBody
	public Object loadRes(@PathVariable String app, String pageid,
			String resFileName) {
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			JavaProject javaProject = ProjectContext
					.getJavaProject(app, pageid);
	...
	@ResponseBody
	public Object saveRes(@PathVariable String app, String pageid,
			String resFileName, String res) {
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			JavaProject javaProject = ProjectContext
					.getJavaProject(app, pageid);
	...
	@ResponseBody
	public Object compile(@PathVariable String app, String pageid,
			String content) {
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			// 获取JavaProject，然后编译
			...
			String compileOutput = javaProject.compile(
					javaCodeInfo.getClassName() + ".java", content);
			map.put("content", compileOutput);
			String className;
			if (StringUtils.isBlank(javaCodeInfo.getPackageName())) {
				className = javaCodeInfo.getClassName();
			} else {
				className = javaCodeInfo.getPackageName() + '.'
						+ javaCodeInfo.getClassName();
			}
	...
	public Object runConsole(@PathVariable String app, String pageid) {
		...
		try {
			...
			JavaProject javaProject = ProjectContext
					.getJavaProject(app, pageid);
			InputStream processInputStream = javaProject
					.getRunProcessInputStream();
	...
	public Object input(@PathVariable String app, String pageid, String input) {
		...
		try {
			...
			if (output != null) {
				IOUtils.write(input + IOUtils.LINE_SEPARATOR_UNIX, output,
						"UTF-8");
				output.flush();
			}
			map.put("success", true);
